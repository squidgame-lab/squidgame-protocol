...
describe('GamePrediction', async () => {
    ...
    describe('#predict', async () => {
        ...
        it('success for predict round 1 num 1 first time', async () => {
            ...
            let userOrders = await gamePrediction.iterateReverseUserRoundOrders(roundId, user1.address, BigNumber.from(0), BigNumber.from(0))
    ...
    describe('#getReward', async () => {
        ...
        it('fails for predicted num is not win number', async () => {
            ...
            await network.provider.send('evm_mine')
            await gamePrediction.setWinNumber(roundId, num1)
            let userOrders = await gamePrediction.iterateReverseUserRoundOrders(roundId, user2.address, BigNumber.from(0), BigNumber.from(0))
