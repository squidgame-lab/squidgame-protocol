 ...
 * `SafeMath128` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library SafeMath128 {
    ...
    function mulAndDiv(uint128 a, uint128 b, uint128 c) internal pure returns (uint128) {
        require(c > 0, "SafeMath128: division by zero");
        if (a == 0 || b ==0) {
            return 0;
        }

        return uint128(uint256(a) * uint256(b) / uint256(c));
    }
}